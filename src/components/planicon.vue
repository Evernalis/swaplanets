                    ...
                    for (let terrcode of biodat) {
                        for (let mateType of this.biomekey) {
                            if (mateType[0].includes(terrcode)){
                                this.biometype.push(new URL('../components/planetimage/material'+mateType[1]+'.png',import.meta.url).href) 
                            } //material URLs follow a common format, this fits the material into the template and is used directly by segment
                        }
                    }
                    if(this.biometype.length==0) {this.biometype[0]=new URL('./components/planetimage/blankplanet.png', import.meta.url).href} // 0 biomes means planet is not shown yet or an error has occured
...
<style scoped>
    .sec0 {
        mask-image: url(./components/planetimage/section0.png);
    }
    .sec1 {
        mask-image: url(./components/planetimage/section1.png);
    }
    .sec2 {
        mask-image: url(./components/planetimage/section2.png);
    }
    .sec3 {
        mask-image: url(./components/planetimage/section3.png);
    }
    .planico {
        width: 128px;
        height: 128px;
